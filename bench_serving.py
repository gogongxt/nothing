...
def get_dataset(args, tokenizer):
    ...
    elif args.dataset_name == "multi-turn-shared-prefix":
        assert not tokenize_prompt
        input_requests = sample_multi_turn_shared_prefix_requests(
            input_len=args.random_input_len,
            output_len=args.random_output_len,
            num_prompts=args.num_prompts,
            range_ratio=args.random_range_ratio,
...
class BenchmarkMetrics:
    ...
    total_cached_tokens: int  # Total cached tokens
...
def sample_multi_turn_shared_prefix_requests(
    input_len: int,
    output_len: int,
    num_prompts: int,
    range_ratio: float,
    ...
    Args:
        ...
        range_ratio: Range ratio for input/output length variation
    ...
    for turn in range(num_turns):
        turn_requests = sample_random_requests(
            input_len=input_len,
            output_len=output_len,
...
def save_requests_to_file(input_requests: List[DatasetRow], filename: str):
    """Save input requests to a JSON file for inspection.

    Args:
        input_requests: List of DatasetRow objects to save
        filename: Path to the output JSON file
    """
    try:
        # Convert DatasetRow objects to dictionaries for JSON serialization
        requests_data = []
        ...
        print(f"Saved {len(input_requests)} requests to {filename}")
    except Exception as e:
...
def calculate_metrics(
    ...
    for i in range(len(outputs)):
        if outputs[i].success:
            ...
            total_cached_tokens += outputs[i].cached_tokens  # Add cached tokens
            if output_len > 1:
...
def run_benchmark(args_: argparse.Namespace):
    ...
    # Save requests to file if specified
    if args.save_requests_to_file:
        save_requests_to_file(input_requests, args.save_requests_to_file)
        print(f"Requests saved to {args.save_requests_to_file}.")
...
if __name__ == "__main__":
    ...
    # Save requests to file without sending them
    parser.add_argument(
        "--save-requests-to-file",
        type=str,
        default=None,
        help="Save generated requests to a JSON file without sending them for inspection (default: None)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Number of conversation turns for multi-turn-shared-prefix dataset (default: 3)",
    )
